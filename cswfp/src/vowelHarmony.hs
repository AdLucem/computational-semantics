...
extractVowel :: [Phoneme] -> Phoneme
extractVowel str = head $ filter (\ x -> (x `elem` yawelmaniVowels)) str
...
appendSuffixY word suffix =
  let
    trigger = extractVowel word
    target = extractVowel suffix
  in
    case ((fValue High trigger) == (fValue High target)) of
      True  -> let
        updatedSuffix suffix =
          let vh x =
                if (fValue x Cons) == Plus
                then
                  x
                else
                  (fMatch Round (fValue Round trigger) (fMatch Back (fValue Back trigger) x))
          in map vh suffix
        in
